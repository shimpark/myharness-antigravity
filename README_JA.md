...
> **myharness は、ドメインを一文で表すだけでエージェントチームとスキルに変換する Claude Code・Codex・Antigravity 3重ランタイムのファクトリーです。**
> `「このプロジェクト用のハーネスを構成して」` の一言で — ドメインを分析し、専門エージェント定義（`.claude/agents/` / `.agents/agents/`）とスキル（`.claude/skills/` / `.agents/skills/`）を、6 つのチームアーキテクチャパターンの中から最適なもので生成します。
...
| **3重ランタイム** | 単一の正本（`skills/myharness/`）+ ランタイム別の薄いアダプター。`CLAUDE.md`／`AGENTS.md`／`skills.json` を一括出力し、オーケストレーションを分岐（Claude は `Agent` チームメンバー spawn ↔ Codex はネイティブ subagents ↔ Antigravity Powershell async）。Phase 7 の同期で drift を防止 |
...
- **エンジンの多様性** — レビュアーはランナーエンジンを **除外** して選択（同じエンジン＝同じ盲点）。Claude Code 実行時は `codex`＋`agy`、Codex 実行時は `claude`＋`agy`、Antigravity 実行時は `claude`＋`codex`。（`agy` = Gemini モデル）
...
your-project/
├── .agents/
│   ├── agents/          # エージェント定義 (ui-developer.md, a11y-reviewer.md …)
│   ├── skills/          # スキル (各 SKILL.md + references/)
│   └── skills.json      # Antigravity スキルマニフェスト
├── .claude/
│   ├── agents/          # Claude Code エージェント定義
│   └── skills/          # Claude Code skill definitions
├── .codex/
│   └── agents/          # Codex TOML エージェント定義
├── CLAUDE.md            # Claude Code エントリポインタ
└── AGENTS.md            # Codex / Antigravity エントリポインタ
```

> **3重ランタイム出力：** 生成時に Antigravity（`.agents/`）、Claude（`.claude/`）、Codex（`.codex/`）互換フォーマットで一括出力・同期します。詳細：`skills/myharness/references/runtime-adapters.md`。

## 3重ランタイム (Claude Code + Codex + Antigravity)

正本（スキル本文・references・スクリプト）は **ランタイム非依存のマークダウン** です。アダプターで分岐するところだけが異なります：

| 関心事 | Claude Code | Codex CLI | Antigravity (Gemini) |
|--------|-------------|-----------|----------------------|
| エントリポイント | `.claude-plugin/plugin.json` + `CLAUDE.md` | `AGENTS.md`（自動ロード） | `AGENTS.md` + `.agents/skills.json` |
| スキル | `.claude/skills/` | `.agents/skills/` | `.agents/skills/` |
| エージェント | `.claude/agents/*.md` | `.codex/agents/*.toml` | `.agents/agents/*.md` または `.claude/agents/*.md` |
| オーケストレーション | `Agent` チームメンバー spawn + `SendMessage` + `TaskCreate` | ネイティブ subagents / `codex exec` | Powershell 非同期 + `manage_task` + `browser_subagent` |
| 外部レビュアー | codex + agy（ランナー claude を除外） | claude + agy（ランナー codex を除外） | claude + codex（ランナー agy を除外） |

> `agy`（antigravity、Gemini モデル）がホストランタイムとして動作する場合、外部レビュアーには `claude+codex` の組み合わせが使用されます。詳細：`skills/myharness/references/runtime-adapters.md`。

## 使用例のプロンプト (Example Prompts)

インストール後、使用中のランタイム（Claude Code、Codex、または Antigravity IDE）にそのまま入力してみてください：

> **💡 Antigravity IDE (Windows) のヒント：**
> UI 開発や Web コンポーネント用のハーネスを実行する際、`browser_subagent` ツールを利用したインタラクティブブラウザのアクセシビリティ・レイアウト検証スキルをオーケストレーターに連動させるよう依頼すると、自動でビジュアルおよびアクセシビリティの検証が構成されます。
...
**A.** Claude Code、Codex、Antigravity (Gemini CLI) の 3 つのランタイムすべてをサポートしています。Markdown の単一正本 (`skills/myharness/`) を維持し、実行時にランタイムアダプターへ分岐します。Antigravity IDE 環境では `AGENTS.md` + `.agents/skills.json` を介してロードされ、Powershell 非同期タスクおよび `browser_subagent` と連携して動作します。詳細：`skills/myharness/references/runtime-adapters.md`。
</details>
