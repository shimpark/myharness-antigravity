...
> **myharness는 도메인 한 문장을 에이전트 팀과 스킬로 바꿔주는 Claude Code·Codex·Antigravity 3중 런타임 팩토리입니다.**
> `"이 프로젝트용 하네스 구성해줘"` 한마디면 — 도메인을 분석해 전문 에이전트 정의(`.claude/agents/` / `.agents/agents/`)와 스킬(`.claude/skills/` / `.agents/skills/`)을, 6가지 팀 아키텍처 패턴 중 맞는 것으로 찍어냅니다.
...
- **출력:** 에이전트 정의 + 스킬 + 오케스트레이터 스킬 + 진입 포인터(`CLAUDE.md`/`AGENTS.md`/`GEMINI.md`)
- **특징:** 한국어 우선 · 슬림 기본(리스크 오를 때만 게이트 강화) · Claude Code/Codex/Antigravity 3중 출력 지원
...
**Antigravity IDE (Gemini CLI / Windows)**
- 프로젝트의 `.agents/skills/` 경로에 `skills/myharness` 폴더를 복사합니다.
- 또는 `.agents/skills.json`을 생성하여 스킬 경로를 명시적으로 등록합니다.
  ```json
  {
    "entries": [
      { "path": ".agents/skills" }
...
### 2. 기동 및 CLI 시작
- **Antigravity IDE:** 대상 프로젝트를 워크스페이스로 열고 대화창을 시작합니다.
- **Claude Code:** 에이전트 팀 모드를 켜고 시작합니다.
  ```shell
...
### 3. 트리거
- **Antigravity IDE:** `"하네스 구성해줘"`, `"하네스 점검해줘"`(설명 기반 implicit 트리거)
- **Claude Code:** `이 프로젝트용 하네스 구성해줘` · `/myharness` · `이 도메인용 에이전트 팀 설계해줘`
- **Codex:** `$myharness` · `/skills` 메뉴 · `하네스 구성해줘`(설명 매칭)
...
| **3중 런타임** | 단일 정본(`skills/myharness/`) + 런타임별 얇은 어댑터. `CLAUDE.md`/`AGENTS.md` 모두 출력, 오케스트레이션 분기(Claude `Agent` spawn ↔ Codex subagents ↔ Antigravity Powershell/manage_task). Phase 7 동기화로 drift 방지 |
| **빌드 하네스 업데이트** | `/myharness update`(Antigravity: `update` 인자 호출) — 팩토리 정본을 이미 빌드된 하네스에 재전파하되 **로컬 수정 보호**. `.harness-manifest.json` 해시 분류(SAME/자동/USER-MODIFIED 보류/NEW), `*.local.*`로 업데이트 안전 |
...
- **엔진 다양성** — 리뷰어는 러너 엔진을 **제외**해 선택(같은 엔진=같은 맹점). Claude Code 실행 시 `codex`+`agy`, Codex 실행 시 `claude`+`agy`, Antigravity 실행 시 `claude`+`codex`. (`agy` = Gemini 모델)
...
your-project/
├── .agents/
│   ├── agents/          # 에이전트 마크다운 정의 (ui-developer.md, a11y-reviewer.md …)
...
├── .codex/
│   └── agents/          # Codex TOML 에이전트 정의
├── CLAUDE.md            # Claude Code 진입 포인터
└── AGENTS.md            # Codex / Antigravity 진입 포인터
```

> **3중 런타임 출력:** 생성 시 Antigravity(`.agents/`), Claude(`.claude/`), Codex(`.codex/`) 호환 포맷으로 일괄 출력 및 동기화합니다. 상세: `skills/myharness/references/runtime-adapters.md`.

## 3중 런타임 (Claude Code + Codex + Antigravity)

정본(스킬 본문·references·스크립트)은 **런타임 무관 마크다운**입니다. 어댑터로 분기하는 것만 다릅니다:

| 관심사 | Claude Code | Codex CLI | Antigravity (Gemini) |
|--------|-------------|-----------|----------------------|
| 진입점 | `.claude-plugin/plugin.json` + `CLAUDE.md` | `AGENTS.md` (자동 로드) | `AGENTS.md` + `.agents/skills.json` |
| 스킬 | `.claude/skills/` | `.agents/skills/` | `.agents/skills/` |
| 에이전트 | `.claude/agents/*.md` | `.codex/agents/*.toml` | `.agents/agents/*.md` 또는 `.claude/agents/*.md` |
| 오케스트레이션 | `Agent` spawn + `SendMessage` + `TaskCreate` | 네이티브 subagents / `codex exec` | Powershell 비동기 + `manage_task` + `browser_subagent` |
| 외부 리뷰어 | codex + agy (러너 claude 제외) | claude + agy (러너 codex 제외) | claude + codex (러너 agy 제외) |

> `agy`(antigravity, Gemini 모델)가 호스트 런타임으로 작동할 경우, 외부 리뷰어는 `claude+codex` 조합이 사용됩니다. 상세: `skills/myharness/references/runtime-adapters.md`.

## 사용 예시 프롬프트

설치 후 사용 중인 런타임(Claude Code, Codex, 또는 Antigravity IDE)에 그대로 입력해 보세요:

> **💡 Antigravity IDE (Windows) 팁:**
> UI 개발이나 웹 컴포넌트 도메인용 하네스를 구동할 때, `browser_subagent` 도구를 활용한 인터랙티브 브라우저 접근성 및 레이아웃 검증 스킬을 오케스트레이터에 연동해 달라고 요청하시면 시각적/동적 검증이 자동으로 구성됩니다.
...
**풀스택 웹 개발**
```
풀스택 웹 개발용 하네스를 만들어줘. 디자인·프론트(Vue.js 3 with CDN)·백엔드(ASP.NET Core 8 MVC, C#, MSSQL, Dapper - Clean Architecture)·QA가
...
**A.** Claude Code, Codex, Antigravity(Gemini CLI) 3개 런타임 모두를 지원합니다. 단일 마크다운 정본(`skills/myharness/`)을 유지하며 각 런타임 어댑터로 분기 기동합니다. Antigravity IDE 환경에서는 `AGENTS.md` + `.agents/skills.json`을 통해 로드되며, Powershell 비동기 태스크와 `browser_subagent`를 연동하여 동작합니다. 상세: `skills/myharness/references/runtime-adapters.md`.
</details>
