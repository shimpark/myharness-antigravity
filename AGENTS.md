...
## 하네스 팩토리 사용
- 새 도메인/프로젝트용 하네스를 만들거나 확장·점검하려면 **`skills/harness/SKILL.md`를 읽고 그 워크플로우(Phase 0~7)를 따른다.**
- Codex 스킬 자동 활성화를 쓰려면 `install.sh`로 `.agents/skills/harness`를 설치한다(그 후 "하네스 만들어줘" 등에 description 기반 활성화 + `/skills`·`$harness` 명시 호출).
...
- 상세: `skills/harness/references/runtime-adapters.md`, 오케스트레이션은 `references/orchestrator-template.md` 템플릿 D.

## 외부 리뷰 게이트
`skills/harness/references/external-review-loop.md` — codex/gemini 독립 검증. 양쪽 런타임 동일하게 subprocess로 작동. `skills/harness/scripts/check-review-tools.sh`로 도구 연동 점검(없으면 게이트 생략).
