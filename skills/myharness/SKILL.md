...
- 에이전트 간 참조 일관성 확인
- 커맨드가 생성되지 않았는지 확인
- **정적 정책 감사:** 팩토리 레포 자체는 `bash scripts/run-policy-audit.sh`로 위 검사(링크 정합·SKILL ≤500·stale 식별자·버전 정합·듀얼런타임 parity·JSON·bash -n)를 일괄 PASS/FAIL 점검(LLM-read 환각 회피). self-evaluation-system.md 증거법 1.
