...
## 목차 (필요한 템플릿만 로드)
- 템플릿 A: 에이전트 팀(기본) · B: 서브 에이전트 · C: 하이브리드 · D: Codex 런타임 어댑터 · E: Antigravity 런타임 어댑터
...
- **템플릿 E: Antigravity 런타임 어댑터** — Antigravity IDE (Gemini) 환경에서 실행 시. run_command 비동기 백그라운드 태스크와 manage_task 툴을 활용한 오케스트레이션 매핑

> 멀티 런타임(Claude Code + Codex + Antigravity) 설계 전반은 `references/runtime-adapters.md` 참조. 오케스트레이터 상단에 "런타임 감지 후 분기"를 명시하고, 각 환경에 맞춰 A, D, E 중 알맞은 템플릿을 따른다.
...
## 템플릿 E: Antigravity 런타임 어댑터

Antigravity IDE (Gemini) 환경에서는 Claude Code 전용 팀 협업 API가 직접 작동하지 않습니다. 대신 `default_api:run_command` 도구의 비동기 실행 흐름과 `manage_task` 툴을 활용하여 백그라운드 태스크 방식으로 병렬 실행 및 오케스트레이션을 제어합니다.

### 매핑 규칙
| 팀 모드(A) | Antigravity 어댑터(E) |
|-----------|----------------------|
| Agent 도구로 팀원 spawn | `run_command` 비동기 실행 (`WaitMsBeforeAsync`를 500ms 정도로 작게 설정하여 명령을 백그라운드로 전송. TaskId 획득) |
| TaskCreate/depends_on | 순차적 혹은 비동기 백그라운드 태스크 관리 (`manage_task` 툴의 `status`를 활용하여 완료 여부 모니터링) |
| SendMessage (팀원 통신) | `_workspace/` 파일로 상태 및 결과 공유 |
| 시각/브라우징 작업 | `default_api:browser_subagent` 도구에 역할을 위임하여 실행 |

### Phase 2: 비동기 백그라운드 실행 (Powershell 예시)
1. 백그라운드로 실행할 개별 에이전트/리뷰어 명령들을 `run_command` 툴을 사용하여 제안합니다.
2. 예시 명령:
   ```powershell
   # WaitMsBeforeAsync: 500
   powershell -ExecutionPolicy Bypass -File .\scripts\agent_runner.ps1 -Agent "builder" -InputPath "_workspace/01_input.md"
   ```
3. `run_command`가 리턴되면, 생성된 백그라운드 `TaskId`를 획득합니다.
4. 여러 개의 작업을 연달아 비동기로 실행하여 병렬(Fan-out) 처리를 구현합니다.

### 태스크 모니터링 및 상태 관리 (백프레셔 & 대기)
- **비동기 대기 규칙**: 백그라운드 태스크가 진행 중일 때, 모델은 지속적으로 `status`를 폴링(polling)하는 대신, 도구 호출을 중단하고 턴을 종료(Idle 상태로 전환)하여 대기합니다.
- 태스크가 완료되면 Antigravity IDE 시스템이 자동으로 완료 통지 메시지를 모델에게 발송하므로, 이때 리액티브하게 깨어나 다음 프로세스를 진행합니다.
- 예외적인 상태 점검이 필요한 경우에만 `manage_task` 툴의 `Action: "status"`를 명시적으로 호출합니다.
- **동시성 캡(Cap)**: 동시에 비동기로 구동하는 태스크 수는 최대 3~5개로 제한합니다.

### 에러 처리 및 좀비 방지
- 백그라운드 태스크가 비정상 지연되거나 무응답(Stale) 상태가 된 경우, `manage_task`의 `Action: "kill"`을 호출하여 작업을 강제 종료시킵니다.
- 실패한 작업은 1회 재시도를 Propose 하되, 지속 실패 시 누락 요소를 최종 결과서에 기록하고 진행합니다.
