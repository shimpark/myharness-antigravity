...
3. **CLAUDE.md(+ Antigravity/Codex는 AGENTS.md)에 하네스 포인터를 등록한다.** — 새 세션에서 오케스트레이터 스킬이 트리거되도록 최소한의 포인터(트리거 규칙 + 변경 이력)만 기록한다. (멀티 출력은 원칙 8·Phase 5-4)
...
8. **멀티 런타임 (Claude Code + Codex + Antigravity).** 각 런타임 거의 대칭. SKILL.md 포맷 동일이라 정본 공유, 어댑터로 분기할 것만: 진입점(plugin.json+CLAUDE.md / AGENTS.md), 스킬 경로(`.claude/skills/` / `.agents/skills/`), 에이전트(`.md` / `.codex/agents/*.toml` / `.agents/agents/*.md`), 오케스트레이션(Agent 팀원 spawn / Codex subagents / Antigravity subagents). 생성 시 양쪽/셋다 출력. 상세·검증: `references/runtime-adapters.md`.
...
- [ ] (멀티 런타임) `.codex/agents/*.toml` 또는 `.agents/agents/*.md` 생성 + `.claude`↔`.codex`/`.agents` 역할 동등성 + `.agents/skills/` references/scripts 동봉 및 `.agents/skills.json` (혹은 standard root 자동 발견) 검증
...
- [ ] (코드/설계) **외부 리뷰어 연동 점검**(`check-review-tools.ps1`/`check-review-tools.sh` — 러너 제외 `REVIEWERS:`) 후 `external-review-loop` 스킬 생성 — 도구 전무면 생략(불필요 스킬 방지) + 단계 게이트 배선, 단계마다 리스크 등급 판정 (Phase 4-6, 5-6)
- [ ] (코드/설계) 커밋 순서·자율 노브(`_workspace/.autonomous`)·push 별도 게이트 반영
- [ ] 결과서에 `## 다음 단계 참조` 블록 (연속성)
- [ ] **멀티 런타임:** 루트 `AGENTS.md` + 스킬 `.agents/skills/` 출력, 오케스트레이터에 어댑터(Agent 팀원 spawn / Codex subagents / Antigravity Powershell 백그라운드) 명시 (`references/runtime-adapters.md`)
